...
\bold{Pname} is the name of the predicate, and $v_i$ is a constant. Let $I_P_i(S)$ be the set of all positive ground literals
produced by $P_i$ at state $S$. $S$ can be described as $S=I_P_1(S)\cup \dots \cup I_P_n(S)$. Thus, the predicates in $\mathbb{P}$
provide a complete description of $S$.

In this work, we are interested in the value function which has the following property:
\[
V(S) = sum_{L_i belong S}i V_L_i,
\]
in which $L_i$ is a positive literal at state $S$. $V_L_i$ is the utility which contributes to the whole 
value function when $L_i$ is positive. The above equation indicates that the value function $V(S)$ can be described
...
%\begin{definition}
%A factored value function is a linear combination of a set of basis functions. 
%\end{definition}
...
in Fig. ?. The world consists of one agent, 5 food and 5 traps.
...
50. the relationship with Peter Stone's Robot Soccer
