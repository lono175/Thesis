...
Though are many batch learning algorithms available for linear regression, they are not suitable for reinforcement learning
...
We denote $V^i$ as the Q-function with the state involved $i$ objects. Our objective is to
...
We objective is to learn the residual function for each training process. If the residual function is 
...
\begin{figure}[h]
    ...
    \begin{minipage}[t]{0.6\linewidth}
        \centering
        \includegraphics[width=\textwidth] {./figures/trap.eps}
        \caption{A grid world with two traps (T). A coin (C) and an agent (A).}
    \label{fig:Trap}
    \end{minipage}
\end{figure}
...
in Fig. \ref{fig:Trap}. The world consists of one agent, $n$ food and $k$ traps.
The number of food and traps as well as the locations are randomly generated at the beginning of each episode.
The agent moves within 8 by 8 grid.
%The size of the state space is $8 \times 8 \times 3 ^ (64 - 1)$
It gets a reward of 10 when it gets a food and -30 if it gets into
a trap. A penalty term -0.1 would apply for each step. If the agent tries to walk out of the world, it will
stay put and receive -1 reward. Each episode ends when all food are consumed or the agent gets into a trap. 
