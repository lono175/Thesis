...
Though are many batch learning algorithms for linear regression available, they are not suitable for reinforcement learning
...
the value for each time step needs to be stored and it becomes impractical as it will eventually
exhausts all available memory resource. Thus, online learning is the only viable option 
for reinforcement learning. 

There are several Least-Squares Temporal Difference (LSTD) algorithms (\cite{LSTD} and \cite{LSPI} which are proposed for 
...
relational setting. Because we don't prior knowledge about which relation is relevant to the value function, 
we provide a bunch of relations which may be large compared to the samples, and these methods are prone
to over-fitting. 
