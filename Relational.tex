...
Assume there exists a constant $a$, $0 < a < 1$, such that $a^iR > \max_x |\Delta V^{i+1}(x)|$ for all $i>1$.
...
Let's change to a different assumption. Assume there are at most $L$ nonzero values for $\Delta V^i$, $i > 1$.
...
Fig. \ref{fig:EasyGame} shows a game which consists of an agent and several food. The agent always starts 
