...
We show the effectiveness of our algorithm on a simple avoid-monster-and-eat-food problem, which is shown
...
Fig. \ref{fig:EasyGame} illustrates a game which satisfies with the assumption of the above theorems. 
...
It satisfy the Theorem 1 but does not satisfy Theorem 2.
...
first order to second order, however, we do not observe the trend continues. This result shows the 
above theorems are sufficient but not necessary conditions for good transfer learning result.
...
and compare the average reward of them. The result is shown in \ref{fig:TemplateOrder}.
Each agent has 10,000 training episodes which are evenly distributed among different templates.
Then we run it for additional 20,000 episodes to test.
